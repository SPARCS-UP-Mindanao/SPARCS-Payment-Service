...
class TransactionUsecase:
    def get_transaction_details(self, get_transaction_details_in: GetTransactionDetailsIn):
        e_wallet_fee_map = {
            EWalletChannels.GCASH.value: 0.023,
            EWalletChannels.PAYMAYA.value: 0.018           
        }
        ...
        if get_transaction_details_in.payment_method == PaymentMethod.E_WALLET.value:
            ...
            transaction_fee = (
                (P * transaction_fee_percentage) if is_fee_greater_than_default else default_fee
            )
            
        equation = Eq(
            P - transaction_fee - (transaction_fee * vat), 
            ticket_price
        )
